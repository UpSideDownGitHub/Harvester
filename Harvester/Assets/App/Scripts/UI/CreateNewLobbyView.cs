...
namespace Assets.App.Scripts.UI
{
    public class CreateNewLobbyView : UIView
    {
        ...
        public void CreateNewMatch()
        {
            ...
            options.Add(Mst.Args.Names.LobbyId, "Harvester"); // I'm not sure if this is correct. -Jason
            options.Add(Mst.Args.Names.RoomMaxConnections, MaxConnections);

            if (!string.IsNullOrEmpty(Password))
                options.Add(Mst.Args.Names.RoomPassword, Password);

            
            MatchmakingBehaviour.Instance.CreateNewLobby("Harvester", options, () =>
            {
                Show();
            });
